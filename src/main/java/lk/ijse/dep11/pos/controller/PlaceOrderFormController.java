...
public class PlaceOrderFormController {
    ...
    public void initialize() throws IOException {
        ...
        txtQty.textProperty().addListener((ov, prev, curQty) -> {
            Item selectedItem = cmbItemCode.getSelectionModel().getSelectedItem();
            btnSave.setDisable(!(curQty.matches("\\d+") && Integer.parseInt(curQty) <= selectedItem.getQty() && Integer.parseInt(curQty) > 0));
    ...
    private void printBill() {
        try {
            ...
            reportParams.put("customer_id", cmbCustomerId.getValue().getId());
            reportParams.put("customer_name", cmbCustomerId.getValue().getName());
    ...
    private void newOrder() throws IOException{
        ...
        try {
            ...
            if(lastOrderId == null){
                ...
                int newOrderId =
                Integer.parseInt(lastOrderId.substring(2)) + 1;
                lblId.setText(String.format("Order ID: OD%03d", newOrderId));
            }
        }catch (SQLException e){
            new Alert(Alert.AlertType.ERROR,
                    "Failed to establish database connection, try later").show();
            e.printStackTrace();
            navigateToHome(null);
        }
